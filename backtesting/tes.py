...
data = pd.read_csv('C:\\Users\\phchi\\Desktop\\python - Serebdipity\\PyStocks\\INFY.csv')  # Replace 'RELIANCE.csv' with the filename of Reliance historical data
...
for i in range(len(data)):
    ...
    strategy_result = strategy.strategy_momentum(stock_data)
    ...
    if strategy_result['Signal'] == 'Buy':
        ...
        while current_index < len(data):
            ...
            if current_price_close >= previous_day_close + (stop * 0.5):
                trailing_stop_loss = trailing_stop_loss + (trailing_stop_loss * 0.5)  # Update trailing stop loss
                previous_day_close = current_price_close
